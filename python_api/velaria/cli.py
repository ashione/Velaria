    ...
    try:
        ...
        if args.command == "source":
            if args.source_command == "create":
                return _source_create_cli(args)
            if args.source_command == "list":
                return _source_list_cli(args)
            if args.source_command == "ingest":
                return _source_ingest_cli(args)
        if args.command == "search":
            if args.search_command == "templates":
                return _search_templates_cli(args)
            if args.search_command == "events":
                return _search_events_cli(args)
            if args.search_command == "datasets":
                return _search_datasets_cli(args)
            if args.search_command == "fields":
                return _search_fields_cli(args)
        if args.command == "grounding":
            return _grounding_cli(args)
        if args.command == "monitor":
            if args.monitor_command == "create":
                return _monitor_create_cli(args)
            if args.monitor_command == "create-from-intent":
                return _monitor_create_from_intent_cli(args)
            if args.monitor_command == "list":
                return _monitor_list_cli(args)
            if args.monitor_command == "show":
                return _monitor_show_cli(args)
            if args.monitor_command == "validate":
                return _monitor_validate_cli(args)
            if args.monitor_command == "enable":
                return _monitor_enable_cli(args)
            if args.monitor_command == "disable":
                return _monitor_disable_cli(args)
            if args.monitor_command == "run":
                return _monitor_run_cli(args)
            if args.monitor_command == "delete":
                return _monitor_delete_cli(args)
        if args.command == "focus-events":
            if args.focus_events_command == "poll":
                return _focus_events_poll_cli(args)
            if args.focus_events_command == "consume":
                return _focus_events_update_cli(args, status_name="consumed")
            if args.focus_events_command == "archive":
                return _focus_events_update_cli(args, status_name="archived")
