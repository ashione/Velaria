...
bazel build //:velaria_pyext //python:bench_realtime_wrapper_stress
...
if command -v leaks >/dev/null 2>&1; then
  ...
  for mode in ${MODES}; do
    leaks -atExit -- "${PYTHON_BIN}" python/benchmarks/bench_realtime_wrapper_stress.py "${LEAK_ITERATIONS}" "${mode}"
  done
else
