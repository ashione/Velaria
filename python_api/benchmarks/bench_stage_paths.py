...
def hardcode_once(csv_path: pathlib.Path):
...
def build_read_csv_kwargs(args, outdir: pathlib.Path):
    if not args.materialization_format:
        return {}
    cache_dir = pathlib.Path(args.materialization_dir) if args.materialization_dir else outdir / "materialization-cache"
...
def write_report(summary: dict, outdir: pathlib.Path, csv_path: pathlib.Path, query: str):
    ...
    for section in ["hardcode", "velaria_full", "velaria_reuse"]:
...
def main():
    ...
    velaria_full_runs = [velaria_full_once(csv_path, args.query, read_csv_kwargs) for _ in range(args.rounds)]
    ...
    velaria_reuse_runs = [velaria_reuse_once(session, df, args.query) for _ in range(args.rounds)]

    summary = {
        "hardcode": summarize_numeric_runs(hardcode_runs, ["elapsed"]),
    ...
    write_report(summary, outdir, csv_path, args.query)
    print(outdir)
