...
def _execute_action_for_run(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "csv-sql":
        output_path = pathlib.Path(args["output_path"]) if args.get("output_path") else artifacts_dir / "result.parquet"
    ...
    if action == "stream-sql-once":
        sink_path = pathlib.Path(args["sink_path"]) if args.get("sink_path") else artifacts_dir / "stream_result.csv"
...
def _run_action_subprocess_target(spec: dict[str, Any], result_path: str, stdout_path: str, stderr_path: str) -> None:
