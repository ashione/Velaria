...
- source pushdown now also classifies 2-3 key `COUNT` and numeric `SUM` / `AVG` as multi-key source shapes; JSON line/array aggregate pushdown uses the shared typed reducer with semantic-safe encoded-key lookup, while CSV and line scanner routing stays on their existing paths because the shared reducer attempts regressed local benchmarks
